...
static inline void s5p_ohci_phy_init(int cmd, struct usb_hcd *hcd)
{

	void __iomem *gpio_regs = (void __iomem *) 0xF8200000;//gpioè™šæ‹Ÿåœ°å€èµ·å§‹åœ°å€ä¸º0xF8200000ï¼Œå°±æ˜¯S5P_VA_GPIO
	u32 tmp;
	tmp = readl(gpio_regs + 0x648);//ETC2PUDå¯„å­˜å™¨åç§»ä¸º0x648
...
static void ohci_s5p_set_clock(int start_clk)
{
...
#ifdef CONFIG_PM
static int s5p_ohci_bus_suspend(struct usb_hcd *hcd)
{
	int ret;
	ohci_s5p_set_clock(1);
	ret = ohci_bus_suspend(hcd);
	ohci_s5p_set_clock(0);
	return ret;
}
static int s5p_ohci_bus_resume(struct usb_hcd *hcd)
{
	int ret;
	ohci_s5p_set_clock(1);
...
#ifdef	CONFIG_PM
	.bus_suspend		= s5p_ohci_bus_suspend,
	.bus_resume		= s5p_ohci_bus_resume,
#endif
...
static int __devinit s5p_ohci_probe(struct platform_device *pdev)
{
	...
	s5p_ohci_phy_init(PHY_CMD_OHCI, hcd);
#endif
